...
function App() {
  ...
  const [autoUpdateInterval, setAutoUpdateInterval] =
    useState<AutoUpdateIntervalMinutes>(DEFAULT_AUTO_UPDATE_INTERVAL);
  ...
  const [resetTimerDisplayMode, setResetTimerDisplayMode] =
    useState<ResetTimerDisplayMode>(DEFAULT_RESET_TIMER_DISPLAY_MODE);
  const [densityMode, setDensityMode] =
    useState<DensityMode>(DEFAULT_DENSITY_MODE);
  const [trayIconStyle, setTrayIconStyle] = useState<TrayIconStyle>(
    DEFAULT_TRAY_ICON_STYLE,
  );
  const [trayShowPercentage, setTrayShowPercentage] = useState(
    DEFAULT_TRAY_SHOW_PERCENTAGE,
  );
  const [globalShortcut, setGlobalShortcut] = useState<GlobalShortcut>(
    DEFAULT_GLOBAL_SHORTCUT,
  );
  ...
  const scheduleTrayIconUpdate = useCallback(
    (_reason: "probe" | "settings" | "init", delayMs = 0) => {
      ...
      trayUpdateTimerRef.current = window.setTimeout(() => {
        ...
        const providerIconUrl =
          style === "provider"
            ? pluginsMetaRef.current.find(
                (plugin) => plugin.id === firstProviderId,
              )?.iconUrl
            : undefined;
  ...
  const displayPlugins = useMemo(() => {
    ...
    return pluginSettings.order
      ...
      .filter((plugin): plugin is { meta: PluginMeta } & PluginState =>
        Boolean(plugin),
      );
    ...
    const resizeWindow = async () => {
      ...
      try {
        ...
        if (monitor) {
          maxHeightPhysical = Math.floor(
            monitor.size.height * MAX_HEIGHT_FRACTION_OF_MONITOR,
          );
      ...
      if (maxHeightLogical === null) {
        ...
        maxHeightLogical = Math.floor(
          screenAvailHeight * MAX_HEIGHT_FRACTION_OF_MONITOR,
        );
      ...
      const height = Math.ceil(
        Math.min(desiredHeightPhysical, maxHeightPhysical!),
      );
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        ...
        let storedDensityMode = DEFAULT_DENSITY_MODE;
        try {
          storedDensityMode = await loadDensityMode();
        } catch (error) {
          console.error("Failed to load density mode:", error);
        }

        const normalizedTrayShowPercentage = isTrayPercentageMandatory(
          storedTrayIconStyle,
        )
          ? true
        ...
        if (isMounted) {
          ...
          setDensityMode(storedDensityMode);
  ...
  const handleResetTimerDisplayModeChange = useCallback(
  ...
  const handleTrayIconStyleChange = useCallback(
    (style: TrayIconStyle) => {
  ...
  const handleTrayShowPercentageChange = useCallback(
    (value: boolean) => {
      track("setting_changed", {
        setting: "tray_show_percentage",
  ...
  const handleDensityModeChange = useCallback((mode: DensityMode) => {
    track("setting_changed", { setting: "density_mode", value: mode });
    setDensityMode(mode);
    void saveDensityMode(mode).catch((error) => {
      console.error("Failed to save density mode:", error);
    });
  }, []);
  ...
  const settingsPlugins = useMemo(() => {
    ...
    return pluginSettings.order
      ...
      .filter(
        (plugin): plugin is { id: string; name: string; enabled: boolean } =>
          Boolean(plugin),
      );
  }, [pluginSettings, pluginsMeta]);
  ...
  const handleToggle = useCallback(
    (id: string) => {
      ...
      track("provider_toggled", {
        provider_id: id,
        enabled: wasDisabled ? "true" : "false",
      });
  ...
  return (
    <div
      ref={containerRef}
      className="flex flex-col items-center p-6 pt-1.5 bg-transparent"
    >
      ...
      <div
        ...
        style={
          maxPanelHeightPx
            ? { maxHeight: `${maxPanelHeightPx - ARROW_OVERHEAD_PX}px` }
            : undefined
        }
      >
